...
def azim_proj(pos):
    ...
    return pol2cart(az, m.cos(elev)*r)
...
def build_cnn(input_var=None, w_init=None, n_layers=(4, 2, 1), n_filters_first=32, imsize=32, n_colors=3):
    ...
    count = 0
    # If no initial weight is given, initialize with GlorotUniform    权重初始化
    if w_init is None:
...
def train(images, labels, fold, model_type, batch_size=32, num_epochs=5):
    ...
    :return: none
    """
    num_classes = len(np.unique(labels))       # 确定几分类
    (X_train, y_train), (X_val, y_val), (X_test, y_test) = reformatInput(images, labels, fold)   # 将数据集的输入和目标加载为numpy数组，分为训练、验证和测试数据
    X_train = X_train.astype("float32", casting='unsafe')    # 输入数据类型转换
...
if __name__ == '__main__':
    ...
    scipy.io.savemat('fold_pairs.mat',{'fold_pairs':fold_pairs})
    # CNN Mode
    print('Generating images...')
    # Find the average response over time windows
    av_feats = reduce(lambda x, y: x+y, [feats[:, i*192:(i+1)*192] for i in range(int(feats.shape[1] / 192))])
    # scipy.io.savemat('av_feats_1.mat', {'av_feats_1.mat':av_feats})
    av_feats = av_feats / (feats.shape[1] / 192)
    # scipy.io.savemat('av_feats_2.mat', {'av_feats_2.mat':av_feats})
    ...
    test1 = np.squeeze(feats[:, -1])

    test2 = np.squeeze(feats[:, -1]) - 1
